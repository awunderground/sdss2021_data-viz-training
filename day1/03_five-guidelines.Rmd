...
```
  labs(
    title = "All three classes had more 
    <span style='color:#00BFC4;'>male passengers</span> than 
    <span style='color:#F8766D;'>female passengers</span>",
    x = NULL,
    y = NULL
  )
```

<font color="#55b748">Step 6:</font> Add `guides(fill = FALSE)` to remove the legend. 
...
tribble(
  ...
  labs(
    title = "All three classes had more <span style='color:#00BFC4;'>male passengers</span> than <span style='color:#F8766D;'>female passengers</span>",
    x = NULL,
    y = NULL
  ) +
  theme_minimal() +  
...
**Tip:** I found [this solution](https://stackoverflow.com/questions/49735290/ggplot2-color-individual-words-in-title-to-match-colors-of-groups) by Googline "add color in ggplot2 title".
...
<font color="#55b748">Step 6:</font> Above the data viz, add `theme_set(theme_minimal())`. This changes the default theme for the entire R session. 
